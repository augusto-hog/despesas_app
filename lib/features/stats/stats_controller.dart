...
class StatsController extends ChangeNotifier {
  ...
  double get minY => _valueSpots.map((e) => e.y).reduce((a, b) => a < b ? a : b);
  double get maxY => _valueSpots.map((e) => e.y).reduce((a, b) => a > b ? a : b);
  double get minX => _periodSpots.map((e) => e.x).reduce((a, b) => a < b ? a : b);
  double get maxX => _periodSpots.map((e) => e.x).reduce((a, b) => a > b ? a : b);
  ...
  void _generateTransactionsByGrouping(
    ...
    for (int i = 0; i < groupingCount; i++) {
      final List<TransactionModel> transactionsByGroup = transactions.where((t) => groupingFunction(i, t)).toList();
