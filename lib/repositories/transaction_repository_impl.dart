...
class TransactionRepositoryImpl implements TransactionRepository {
  ...
  Future<DataResult<bool>> addTransaction({
    ...
    try {
      final newTransaction = transaction.copyWith(userId: userId, syncStatus: SyncStatus.create).toDatabase();
  ...
  Future<DataResult<BalancesModel>> getBalances() async {
    try {
      final balanceResponse = await databaseService.read(path: TransactionRepository.balancesPath);
      BalancesModel cachedBalances = BalancesModel.fromMap((balanceResponse['data'] as List).first);
  ...
  Future<DataResult<bool>> updateTransaction(
    ...
    try {
      ...
      await syncService.saveLocalChanges(
        path: TransactionRepository.transactionsPath,
        params: transaction.copyWith(syncStatus: SyncStatus.update).toDatabase(),
      );
  ...
  @override
  Future<DataResult<bool>> deleteTransaction(TransactionModel transaction) async {
    try {
      await updateBalance(transaction);
      ...
      await syncService.saveLocalChanges(
        path: TransactionRepository.transactionsPath,
        params: transaction.copyWith(syncStatus: SyncStatus.delete).toDatabase(),
      );
  ...
  Future<void> updateBalance(newTransaction) async {
    ...
    final oldTransaction = transactionExists ? TransactionModel.fromMap((transactionMap['data'] as List).first) : null;

    final balanceMap = await databaseService.read(path: TransactionRepository.balancesPath);
