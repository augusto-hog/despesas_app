...
class SyncService {
  ...
  Future<void> _syncTransactionsFromServer() async {
    ...
    clock.stop();
    log('total sync time: ${clock.elapsed.inSeconds}', name: 'Sync Transactions from Server');
  }

  // Sync balance from server to local database
  Future<void> _syncBalanceFromServer() async {
    log('_syncBalance called', name: 'INFO');
    final localBalanceResponse = (await databaseService.read(path: TransactionRepository.balancesPath));

    if ((localBalanceResponse['data'] as List).isNotEmpty) return;

    final remoteBalanceResponse = await graphQLService.read(path: Queries.qGetBalances);
    ...
    try {
      for (final t in localTransactions) {
        ...
        if (t.syncStatus == SyncStatus.delete) {
          databaseService.delete(path: TransactionRepository.transactionsPath, params: {'id': t.id});
  ...
  Future<List<TransactionModel>> _getLocalTransactions() async {
    final response = await databaseService.read(path: TransactionRepository.transactionsPath);
    ...
    final localChanges = parsedTransactions.where((t) => t.syncStatus != SyncStatus.synced).toList();
